...
test_that("calculate_asr_direct validates value ranges correctly", {
  ...
  # Test zero person_years - should work with warning and exclude those groups
  ...
  expect_warning(
    result_zero_py <- calculate_asr_direct(data_zero_py),
    "Excluding 1 age group with zero person-years"
  )

  # Should have 2 age groups remaining (excluded the 0-20 group)
  expect_equal(nrow(result_zero_py$age_specific_data[[1]]), 2)
  expect_false("0-20" %in% result_zero_py$age_specific_data[[1]]$age_group)

  # Test all zero person_years - should error
  data_all_zero_py <- data.frame(
    age_group = c("0-20", "20-40", "40-60"),
    events = c(5L, 15L, 25L),
    person_years = c(0, 0, 0),
    standard_pop = c(1000, 1500, 2000)
  )

  expect_error(
    suppressWarnings(calculate_asr_direct(data_all_zero_py)),
    "No age groups remain after excluding those with zero person-years"
  )

  # Test zero person_years with warnings disabled - should work silently
  expect_no_warning(
    result_silent <- calculate_asr_direct(data_zero_py, warn_small_cases = FALSE)
  )
  expect_equal(nrow(result_silent$age_specific_data[[1]]), 2)
  ...
  expect_error(
    ...
    "Column 'person_years' must contain non-negative values"
  )
